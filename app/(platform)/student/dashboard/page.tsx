...
import { CheckCircle, Clock, Award, Bell } from "lucide-react";
import { ActivityChart } from "@/components/charts/activity-chart";
import { AddActivityModal } from "@/components/modals/add-activity-modal";
import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar";
import { DownloadPortfolioModal } from "@/components/modals/download-portfolio-modal";
...
const ActivityItem = ({
  ...
  return (
    <div className="flex items-center gap-4 border-b pb-4 last:border-b-0 last:pb-0 bg-white p-3 rounded-lg">
      ...
      <div
        className={`ml-auto rounded-full px-3 py-1 text-xs font-medium ${statusBadgeStyle[status]}`}
      >
        {status}
      </div>
    </div>
...
export default async function StudentDashboard() {
  ...
  return (
    <div className="relative min-h-full">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-right">
            <div>
              ...
              <p className="font-semibold">{profileData?.students?.course}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Year</p>
              <p className="font-semibold">{profileData?.students?.year}</p>
            </div>
            <div>
      ...
      {/* --- THIS IS THE FIX: Changed from flex-col to flex-row --- */}
      <div className="fixed bottom-8 right-8 z-50 flex flex-row gap-4">
